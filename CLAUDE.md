...
Default models: Haiku 4.5 (Claude), Grok 4.3 (xAI). Keep Grok 4.6 in the picker as the escalate option for deeper / stuck logs. `settingsRev` 2 is a one-time migrate of stored `xaiModel` grok-4.6 → grok-4.3; persist must keep `settingsRev` so an explicit 4.6 choice sticks.
