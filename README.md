...
- **Health states**: 🔴 **Management broken** (cert expired) · 🟠 **At risk** (expires ≤ 30 days) · 🟡 **Stale** (no check-in ≥ 14 days) · 🟢 **Healthy**.
- **Risk column**: **High** = expired cert *and* synced within 7 days — the device still checks in and looks fine while management is dead, the most dangerous state because admins assume it's healthy. **Medium** = expired-but-silent or expiring soon; **Low** = stale.
- **Click any status badge** for a per-device management assessment: why it was classified, a confidence statement, expected symptoms (pending Win32 installs, scripts never executing, hourly IME auth retries), and a suggested investigation runbook (ClientCertCheck.log, IntuneManagementExtension.log, `RenewStatus`/`RenewErrorCode`, certlm.msc, re-enroll).
- **KPI tiles**: Windows devices in scope · Management broken · At risk · Stale · Management health rate gauge. Tiles click-filter the table.
...
- Device names deep-link to the device's Intune blade; per-row **⚡ Check-in** forces an IME sync.
...
- `DeviceManagementManagedDevices.PrivilegedOperations.All` — the per-device **⚡ Check-in** action (Hardware / Failed Install / Management health tabs), which fires `initiateOnDemandProactiveRemediation` to force an IME required-app sync; requires an Intune Administrator role.
