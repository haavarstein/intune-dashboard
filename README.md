...
- `Mail.Send` — send MAA notification emails from your own mailbox: the approver notification at the moment a delete is submitted, and the requestor "approved — action required" notification when you approve a request from the Approvals sub-tab; never used for anything else. All MAA emails share one Microsoft-notification-style template (white body, black button, Settings/Value table)
