...
Because the preview's hunting schema is in flux, the tab **cascades through three data sources** and a banner states which one answered: **`AgentsInfo`** (the replacement table — tried first) → **`AIAgentsInfo`** (SlimKQL's original, retired **July 1, 2026**) → the **exposure graph** (`ExposureGraphEdges`, `endpointAiAgent` → *runs on* → device — Microsoft's documented home for endpoint agent discovery; carries agent + device only, so vendor/user/timestamp columns show "—" from that source). A 400 from a source means the table doesn't exist in the tenant and the next is tried; if none resolve, the tab explains that the discovery preview hasn't reached the tenant. Zero rows everywhere renders a checklist of discovery prerequisites instead of a silent empty table.
