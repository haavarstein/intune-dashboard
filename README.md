...
- **+ Deploy a remediation.** A toolbar button opens a form to create a Proactive Remediation (`deviceHealthScript`) in the active tenant from your own scripts — paste or upload a **detection** `.ps1` (must `exit 1` when the issue is found) and an optional **remediation** `.ps1`, pick **Run as** (System for HKLM/machine, logged-on user for HKCU/per-user), **64-bit/32-bit**, and signature-check, then assign **On-demand only** (unassigned), to a **group**, or **All Devices** with an **Hourly/Daily** schedule. Scripts are sent UTF-8; first use needs `DeviceManagementScripts.ReadWrite.All`; Multi-Admin Approval is handled. Authoring is left to you — for registry keys, [chrispro.tech's Registry → Remediation](https://chrispro.tech/tools/registry-to-remediation) generates both scripts client-side (keeping detection output under Intune's 2,048-char cap); paste them straight in. The new remediation then shows in the list above.
- **⚡ IME Required App Check-in — run on a device of choice (on-demand).** A card at the top of the tab lets you **search for a Windows device and force a required-app check-in on it**. It runs Rudy Ooms / Call4Cloud's [Required-App-Checkin](https://github.com/call4cloud-code/Required-App-Checkin-public) remediation, which calls the Intune Management Extension's internal `IStatusService.CheckInAsync` to kick the **required + available apps** path — cutting the well-known ~60-minute delay for required Win32 apps after Autopilot or a fresh assignment. This is a **device-targeted tool, not a scheduled or group-assigned rollout.** The same **⚡ Check-in** action also appears on every device row in the **Hardware**, **Failed Install**, and **Cert health** tabs (these problems overlap — a stuck cert or a failed required install is exactly when you want to force a sync).
...
- The **+ Deploy a remediation** feature (Remediation tab) was inspired by Chris Procter's client-side [Registry → Remediation](https://chrispro.tech/tools/registry-to-remediation) generator — the dashboard adds the deploy-to-tenant half; the dashboard ships no code from it.
