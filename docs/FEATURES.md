...
Deep documentation for every tab, Graph call, and edge case. For a short overview and quick start, see the root [README.md](../README.md).

This file is the former long README, kept so operational detail is not lost when the root README was shortened.
...
**Required Install** — Win32 apps assigned as *Required* to *All Devices* or *All Users*. **Column table**: App · Publisher · Platform · Context (SYSTEM/USER) · Target · Open. Filters: target, install context, name/publisher search. Click app name / Open for the Intune admin-center blade.
...
**Required Uninstall** — apps assigned with intent *Uninstall* to a group. Same **column table** as Required Install (App · Publisher · Platform · Context · Groups · Open).
- Alphabetical list of `displayName`s. Click any row to open the app's blade in the Intune admin center in a new tab.
- **Platform filter** dropdown defaults to *Windows*; switch to *All*, *Android*, *iOS*, or *macOS* as needed.
- Type to filter the list.
...
- **Bulk delete**: checkboxes on every package row (plus select-all in the header) reveal a **🗑 Delete Selected (n)** button — confirm once with a package preview, and the dashboard deletes them serially. MAA tenants get one justification prompt reused for the whole batch; packages that come back 412/409 are reported as **⏳ queued for Multi-Admin Approval** (with a combined approver email if the customer has approvers configured), successes and failures are itemised in the result banner.
...
- **Haiku 4.5 by default** (Claude) or **Grok 4.3** (xAI) — switch provider and model in Settings. Escalate to Grok 4.6 for stuck logs.
...
- `DeviceManagementApps.ReadWrite.All` — deleting apps from Intune (Installed sub-tab → 🗑 Delete from Intune, App Versions drill-down → per-package 🗑 and bulk 🗑 Delete Selected) and the Approvals sub-tab's approve / reject / cancel actions plus completing approved app deletes
- `Mail.Send` — send MAA notification emails from your own mailbox: the approver notification at the moment a delete is submitted, and the requestor "approved — action required" notification when you approve a request from the Approvals sub-tab; never used for anything else. All MAA emails share one Microsoft-notification-style template (white body, black button, Settings/Value table, grey footer band) built as nested-table HTML mirroring a real PIM notification so it renders correctly in Outlook's Word-based engine
...
- `DeviceManagementManagedDevices.ReadWrite.All` — the Management health tab's **🗑 Delete selected** / **⚠ Wipe selected** bulk device actions, and completing approved device wipe / retire / delete requests from the Approvals sub-tab
- `DeviceManagementRBAC.ReadWrite.All` — completing an approved MAA **access-policy** create from the Approvals sub-tab
- `DeviceManagementConfiguration.ReadWrite.All` — completing approved script / remediation / compliance / settings-catalog / device-category requests from the Approvals sub-tab

Everything requested at sign-in is read-only; all write scopes are just-in-time. Stricter tenants may require admin consent for the write scopes; if you can't consent yourself, an Intune admin needs to grant it before 🗑 Delete from Intune, the approver-notification email, ⚡ Auto-deploy, ↻ Restore (Soft-deleted sub-tab), and ↻ Revoke / ⊘ Disable (Stale users sub-tab) will work.
...
- `DELETE /beta/deviceAppManagement/mobileApps/{id}` — permanently delete an app from the tenant (Installed sub-tab → 🗑 Delete from Intune, and App versions sub-tab → per-package 🗑 or bulk 🗑 Delete Selected). Also called with the `x-msft-approval-code` header to complete an approved app-delete request from the Approvals sub-tab
...
- `POST /v1.0/me/sendMail` — send the MAA approver-notification email from your mailbox when a delete submission triggers HTTP 412/409 and the active customer has approvers configured, and the requestor "approved — action required" email when you approve a request from the Approvals sub-tab
...
- `POST /beta/deviceManagement/operationApprovalRequests/{id}/approve` and `…/reject` — inline Approve/Reject actions from the Approvals sub-tab (JIT `DeviceManagementApps.ReadWrite.All` scope)
...
- `POST` / `PATCH` / `DELETE` replay against `operationApprovalPolicies`, `deviceManagementScripts`, `deviceHealthScripts`, `deviceCompliancePolicies`, `configurationPolicies`, and `deviceCategories` with the same `x-msft-approval-code` header — Complete approved non-device requests; the stored `payload` / `payloadId` from the approval record is replayed
...
- **Code** — required, 2–4 letters (e.g. `ACME`, `FAB`). The code is the *only* identifier that shows up in the dashboard's top-right tenant dropdown, so customer names stay off screenshots, recordings, and over-the-shoulder views.
