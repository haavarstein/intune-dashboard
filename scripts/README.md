...
You pick the device either from the search box on the **Remediation** sub-tab or with the **⚡ Check-in** button on any row in the **Hardware** or **Failed Install** tabs (on the **Management health** tab it lives inside the per-device Health Assessment panel). The created remediation carries no assignment and no schedule — it exists solely as the vehicle for these per-device runs.
